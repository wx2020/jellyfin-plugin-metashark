...
namespace Jellyfin.Plugin.MetaShark.Providers
{
    ...
    {
        public SeriesProvider(IHttpClientFactory httpClientFactory, ILoggerFactory loggerFactory, ILibraryManager libraryManager, IHttpContextAccessor httpContextAccessor, DoubanApi doubanApi, TmdbApi tmdbApi, OmdbApi omdbApi, ImdbApi imdbApi, MoviePilotApi? moviePilotApi = null)
            : base(httpClientFactory, loggerFactory.CreateLogger<SeriesProvider>(), libraryManager, httpContextAccessor, doubanApi, tmdbApi, omdbApi, imdbApi, moviePilotApi)
        {
        }
        ...
        public async Task<IEnumerable<RemoteSearchResult>> GetSearchResults(SeriesInfo info, CancellationToken cancellationToken)
        {
            ...
            var mpSearch = await this.SearchMoviePilotAsync(info.Name, false, cancellationToken).ConfigureAwait(false);
            var res = mpSearch.Count > 0
                ? mpSearch
                : await this._doubanApi.SearchTVAsync(info.Name, cancellationToken).ConfigureAwait(false);
        ...
        public async Task<MetadataResult<Series>> GetMetadata(SeriesInfo info, CancellationToken cancellationToken)
        {
            ...
            if (metaSource != MetaSource.Tmdb && !string.IsNullOrEmpty(sid))
            {
                ...
                var subject = await this.GetMoviePilotSubjectAsync(sid, false, cancellationToken).ConfigureAwait(false);
                if (subject == null)
                {
